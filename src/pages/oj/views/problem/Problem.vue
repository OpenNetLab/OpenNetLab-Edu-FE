      ...
      <Card :padding="20" id="submit-code" v-if="!hideCodeMirror" dis-hover>
        ...
        <Row type="flex" justify="space-between">
          <Col :span="10">
            ...
            <div v-else-if="this.contestID && submissionExists">
            ...
            <div v-if="submit_ban">
              <Alert type="warning" show-icon>{{$t('m.Submission_limit')}}</Alert>
            </div>
          </Col>
          ...
          <Col :span="12">
            ...
            <Button type="warning" icon="edit" :loading="submitting" @click="submitCode"
                    :disabled="problemSubmitDisabled || submitted || submit_ban"
                    class="fl-right">
    ...
    data () {
      return {
        ...
        problem: {
          ...
          code_description: ['test1', 'test2'],
          tags: ['tcp', 'reliable transmission'],
      ...
      submitCode () {
        for (let code of this.code_value_list) {
          if (code.value.trim() === '') {
            ...
            this.code_list.push(code.value)
          }
        }
        this.submissionId = ''
          ...
          api.submitCode(data).then(res => {
            ...
            if (!detailsVisible) {
              this.$Modal.success({
                ...
                onOk: () => {
                  if (this.contestID) {
                    return this.$router.push({name: 'contest-submission-list', query: {problemID: this.problemID}})
                  } else {
                    return this.$router.push({name: 'submission-list', query: {problemID: this.problemID}})
                  }
                }
              })
              // if (res.data.data.contest === undefined) {
              //   this.$router.push('/status')
              // } else {
              //   this.$router.push('/lab/' + res.data.data.contest + '/submission')
              // }
              // return
            }
            this.submitted = true
            ...
            //   this.$router.push('/status')
            // } else {
            //   this.$router.push('/lab/' + data.contest_id + '/submission')
            // }
          }, res => {
            this.getCaptchaSrc()
