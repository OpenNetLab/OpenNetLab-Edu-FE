...
<script>
  ...
  export default {
    ...
    data () {
      ...
      const CheckEmailInUSTC = (rule, value, callback) => {
        api.checkEmailFromUSTC(value).then(res => {
          if (res.data.data.email !== true) {
            callback(new Error(this.$i18n.t('m.UstcEmail')))
          } else {
            callback()
          }
        }, _ => callback())
      ...
      return {
        ...
        ruleRegister: {
          ...
          email: [
            ...
            {validator: CheckEmailInUSTC, trigger: 'blur'}
          ],
          password: [
