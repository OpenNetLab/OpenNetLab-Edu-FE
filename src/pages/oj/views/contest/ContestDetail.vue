...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          {
            title: this.$i18n.t('m.Tutors'),
            render: (h, data) => {
              return h('span', data.row.contest_admin)
            }
          },
          {
    ...
    watch: {
      '$route' (newVal) {
        this.route_name = newVal.name // e.g. 点击Submissions，路由改变为{name: 'contest-submission-list'}
        this.contestID = newVal.params.contestID  // e.g. 还是contestID为1
